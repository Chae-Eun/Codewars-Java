...
public class FindOutlier{
    static int find(int[] integers){
        ...
        if((int)Arrays.stream(integers).filter(n -> n%2 == 0).count() == 1) // even
            num = Arrays.stream(integers).filter(n -> n % 2 == 0).findFirst().getAsInt();
        else // add
            num = Arrays.stream(integers).filter(n -> Math.abs(n % 2) == 1).findFirst().getAsInt();
